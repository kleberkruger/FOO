...
public class ResponsiveHandler {
    ...
    private static InvalidationListener MANAGED_LISTENER = new InvalidationListener() {
    ...
    private static void updatePseudoClasses(Node n, DeviceType type) {
        type.getInactiveClasses().forEach((pseudoClass) -> {
            n.pseudoClassStateChanged(pseudoClass, false);
        });
        type.getActiveClasses().forEach((pseudoClass) -> {
            n.pseudoClassStateChanged(pseudoClass, true);
        });
        DEVICE_TYPE_PROPERTY.set(type);
    }

    private static void removeAllPseudoClasses(Node n) {
        DeviceType.getAllClasses().forEach((pseudoClass) -> {
            n.pseudoClassStateChanged(pseudoClass, false);
        });
    }

    // ******************** Event handling *************************************
    public static final ObjectProperty<DeviceType> DEVICE_TYPE_PROPERTY = new ObjectPropertyBase<DeviceType>(DeviceType.NONE) {
        @Override
    ...
    public static final void setOnDeviceTypeChanged(final InvalidationListener LISTENER) {
        DEVICE_TYPE_PROPERTY.addListener(LISTENER);
    }
